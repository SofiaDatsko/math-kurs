...
// Завантаження дозволених курсів для студента
async function loadUserAccess() {
    if (!currentUser || currentUser.role === 'teacher') return;
    try {
        const snapshot = await db_cloud.collection('course_requests')
            .where('studentEmail', '==', currentUser.email)
            .where('status', '==', 'approved')
            .get();
        myApprovedCourses = snapshot.docs.map(doc => doc.data().courseId);
        renderHome(); 
    } catch (e) { console.error("Access load error:", e); }
}
...
auth.onAuthStateChanged(async (user) => {
    if (user) {
        ...
        if (currentUser.role === 'student') await loadUserAccess();
...
function handleGoogleLogin() {
    ...
    auth.signInWithPopup(provider).catch(err => alert('Помилка Google: ' + err.message));
}
...
function renderLesson(cid, tid) {
    ...
    const matItems = t.materials.map(m => `
        <li class="hw-item">
            ...
            <a href="${m.url !== '#' ? m.url : '#'}" target="_blank" style="text-decoration:none; color:inherit;">
                ${esc(m.title)}
            </a>
        </li>
    `).join('');
            ...
            presContent = `
                <div style="text-align:center; padding:40px; background:var(--bg); border-radius:12px;">
                    <a href="${esc(t.presUrl)}" target="_blank" class="btn-primary" 
                       style="padding:15px 30px; font-size:1.1rem; text-decoration:none; display:inline-block; border-radius:10px; cursor:pointer;">
                       🖥 Відкрити презентацію в новому вікні
                    </a>
                </div>`;
        }
    }
...
function renderAdmin() {
    ...
    html += `<button class="nav-btn add-btn" data-action="add-course">+ Новий курс</button>`; 
    nav.innerHTML = html;
...
function renderAdminContent() {
    ...
    if (adminLayoutState.section === 'progress') {
        ...
        db_cloud.collection("student_results").orderBy("date", "desc").get()
            .then((querySnapshot) => {
    ...
    if (adminLayoutState.section === 'course') {
        ...
        document.getElementById('as-course-save').onclick = () => { 
            ...
            saveDB(db); toast('✓ Курс збережено!'); renderAdmin(); 
        };
        document.getElementById('as-course-del').onclick = () => { 
            if (!confirm('Видалити весь курс?')) return; 
            db.courses = db.courses.filter(curr => curr.id !== adminLayoutState.courseId); 
            adminLayoutState = {section: 'courses', courseId: null, topicId: null}; 
            saveDB(db); toast('Курс видалено'); renderAdmin(); 
        };
    } 
